...
function Orders() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      {orders.map((order) => (
        <div key={order.id} style={{ background: "#fff", padding: "16px", borderRadius: "10px", boxShadow: "0 2px 6px rgba(0,0,0,0.08)" }}>
          <p style={{ fontWeight: "bold", marginBottom: "8px" }}>
            Order #{order.id} — <span style={{ color: "green" }}>{order.status}</span>
          </p>
